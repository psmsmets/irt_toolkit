...
# Ray trace options
echo "0 50 1" >> $tmpfile		# elevation range (min, max and stepsize)
                                        # 0º, 90º (zenith)
echo "0 360 2" >> $tmpfile		# azimuth range (min, max and stepsize)
                                        # 0º N, 90ºE, 180ºS, 270ºW
echo "1" >> $tmpfile			# transmission loss frequency (Hz)
...
echo "1" >> $tmpfile			# ray data output type (1=reflections,2=rays,3=both)
echo "350.5 5.0 0.5 0 T" >> $tmpfile	# vertical cross-section: bearing (deg), horizontal and vertical resolution (km),
					# number of rays to skip in plot (-)
					# type 'no' for no output
echo "yes" >> $tmpfile			# verbose (no/yes)

# Stop conditions
echo "ll 49.0 54.5 2.5 8.5" >> $tmpfile		
